    ...
    if (selectedLayer) {
        ...
        if (layer) {
            ...
            if (layerName.startsWith('dsview')) {
                // Remove o prefixo 'dsview:' para construir o URL de download corretamente
                var cleanLayerName = layerName.replace('dsview:', '');

                // Executa o código que usa o Django
                        ...
                        .then(data => {
                            if (data.features && data.features.length > 0) {
                                data.features.forEach(function(feature) {
                                    ...
                                    var downloadLink_shapefile = `/wri/download/shapefile/${cleanLayerName}/${featureId}/`;
                                    var downloadLink_geojson = `/wri/download/geojson/${cleanLayerName}/${featureId}/`;
                                    var downloadLink_csv = `/wri/download/csv/${cleanLayerName}/${featureId}/`;
                                    var downloadLink_gml = `/wri/download/gml32/${cleanLayerName}/${featureId}/`;
                                    var downloadLink_kml = `/wri/download/kml/${cleanLayerName}/${featureId}/`;
                                    var downloadLink_gpc = `/wri/download/geopackage/${cleanLayerName}/${featureId}/`;
                                    infoContent += `<div class="download-container">`;
                                    infoContent += `<p><a href="${downloadLink_shapefile}" target="_blank" class="download-link">Shapefile</a></p>`;
                                    infoContent += `<p><a href="${downloadLink_geojson}" target="_blank" class="download-link">Geojson</a></p>`;
                                    infoContent += `<p><a href="${downloadLink_csv}" target="_blank" class="download-link">CSV</a></p>`;
                                    infoContent += `<p><a href="${downloadLink_gml}" target="_blank" class="download-link">GML 3.2</a></p>`;
                                    infoContent += `<p><a href="${downloadLink_kml}" target="_blank" class="download-link">KML</a></p>`;
                                    infoContent += `<p><a href="${downloadLink_gpc}" target="_blank" class="download-link">Geopackage</a></p>`;
                                    infoContent += `</div>`;
